...
from common.utils import non_max_suppression, put_chinese_text
    ...
    def multi_target_matching(self, templates, search_img, threshold):
        ...
        try:
            ...
            for i, (template_img, filename, color) in enumerate(templates):
                ...
                if h_t > h_s or w_t > w_s:
                    results.append({"filename": filename, "status": "skip", "reason": "尺寸过大"})
                    continue
            ...
            logger.info(
                f"多目标匹配完成，成功匹配{len([r for r in results if r['status'] == 'success'])}个目标"
            )

            # 封装结果
            return {"results": results, "search_size": (w_s, h_s), "threshold": threshold}
        except Exception as e:
