...
class FeatureTab(ctk.CTkFrame):
    ...
    def build_control(self):
        ...
        ctk.CTkButton(
            self.data_card.content, text="加载图像", command=self.load_image
        ).pack(fill="x", pady=3, padx=5)
        ctk.CTkButton(
            self.data_card.content, text="保存结果", command=self.save_result
        ).pack(fill="x", pady=3, padx=5)
        ctk.CTkButton(
            self.data_card.content, text="重置视图", command=self.reset_view
        ).pack(fill="x", pady=3, padx=5)
        ...
        ctk.CTkLabel(self.geom_card.content, text="插值方法", font=FONT_NORMAL).pack(anchor="w", pady=(5,2), padx=5)
        ctk.CTkOptionMenu(
        ...
        ctk.CTkLabel(self.feature_card.content, text="检测算法", font=FONT_NORMAL).pack(anchor="w", pady=(5,2), padx=5)
        ctk.CTkOptionMenu(
            self.feature_card.content, variable=self.feature_method,
        ...
        self.add_slider_entry(self.thresh_frame, "检测阈值", self.cur_thresh,
        ...
        # 显示开关
        ctk.CTkCheckBox(
            self.feature_card.content, text="显示特征点", variable=self.enable_feature, font=FONT_NORMAL
        ...
        self.count_label = ctk.CTkLabel(
            self.stat_card.content, text="检测到特征点：0", text_color=THEME["success"], font=FONT_SUBTITLE
        )
    ...
    def bind_realtime_render(self):
        """实时渲染绑定：参数变化自动重新计算"""
        vars_list = [self.angle, self.scale_ratio, self.interp_method,
                    self.feature_method, self.point_size, self.enable_feature,
                    self.harris_k, self.susan_t, self.cur_thresh]
        for v in vars_list:
    ...
    def render(self, *args):
        ...
        try:
            if method == "harris":
                mask, cnt = self.detector.harris_detect(gray, self.harris_k.get(), self.cur_thresh.get())
            elif method == "moravec":
                ...
                mask, cnt = self.detector.susan_detect(gray, self.susan_t.get(), self.cur_thresh.get())
    ...
    def save_result(self):
        ...
        if self.result_img is None:
            ...
            return
        path = filedialog.asksaveasfilename(defaultextension=".png", filetypes=[("PNG图像","*.png"),("JPG图像","*.jpg")])
        if path:
            try:
