...
import cv2
import os
from .theme import THEME, FONT_SUBTITLE, FONT_NORMAL, FONT_SMALL, PANEL_STYLE, CollapsibleCard
from common.config import DEFAULT_VECTOR_COLOR, SELECTED_COLOR
from common.logger import logger
from common.utils import set_chinese_font, safe_execute
from data import read_shp, save_shp, save_dwg
from core import (
    move_feature, create_new_layer, select_feature,
    create_point_feature, create_line_feature, create_polygon_feature,
    update_feature_property, add_property_field, delete_property_field,
...
class VectorTab(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        ctk.CTkButton(
            self.file_card.content, text="加载SHP文件", command=self.load_shp
        ...
        ctk.CTkButton(
            self.file_card.content, text="导出文件", command=self.export_file
        ).pack(fill="x", pady=3, padx=5)
        ...
        tools = [("选择要素","select"),("移动要素","move"),("绘制点","draw_point"),
                 ("绘制线","draw_line"),("绘制面","draw_polygon")]
        for t, v in tools:
            ctk.CTkRadioButton(
                self.edit_card.content, text=t, variable=self.tool_var, value=v,
                command=self.set_edit_mode, font=FONT_NORMAL
            ).pack(anchor="w", pady=2, padx=5)
        
        ctk.CTkButton(
            self.edit_card.content, text="删除选中", fg_color=THEME["danger"], command=self.delete_selected
        ).pack(fill="x", pady=5, padx=5)
        ctk.CTkButton(
            self.edit_card.content, text="清空画布", command=self.clear_all
        ).pack(fill="x", pady=3, padx=5)
        ...
        # 图层树
        self.layer_tree = ttk.Treeview(self.layer_card.content, columns=("name", "type"), show="headings", height=5)
        ...
        ctk.CTkButton(layer_btn_frame, text="显示", width=6, command=self.show_layer).pack(side="left", padx=1)
        ctk.CTkButton(layer_btn_frame, text="隐藏", width=6, command=self.hide_layer).pack(side="left", padx=1)
        ctk.CTkButton(layer_btn_frame, text="删除", width=6, command=self.delete_layer).pack(side="left", padx=1)
        
        # 4. 属性编辑
        self.prop_card = CollapsibleCard(self.control_scroll, "属性编辑")
        self.prop_card.pack(fill="x", pady=5, padx=5)
        
        self.prop_tree = ttk.Treeview(self.prop_card.content, columns=("field","value"), show="headings", height=6)
        ...
        ctk.CTkButton(prop_btn_frame, text="修改", width=6, command=self.edit_prop).pack(side="left", padx=1)
        ctk.CTkButton(prop_btn_frame, text="新增", width=6, command=self.add_field).pack(side="left", padx=1)
        ctk.CTkButton(prop_btn_frame, text="删除", width=6, command=self.del_field).pack(side="left", padx=1)
        
        ctk.CTkButton(
            self.prop_card.content, text="批量赋值", fg_color=THEME["accent"], command=self.batch_edit_prop
        ).pack(fill="x", pady=3, padx=5)
        
        # 提示信息
        self.tip_label = ctk.CTkLabel(
            self.control_scroll, text="选择要素查看属性", wraplength=250,
    ...
    def set_edit_mode(self):
        ...
        self.drawing_points = []
        self.clear_temp()
        tips = {"select":"点击选择要素","move":"拖动移动要素","draw_point":"点击绘制点",
                "draw_line":"双击结束绘制","draw_polygon":"双击闭合面"}
        self.tip_label.configure(text=tips[self.edit_mode])
        self.status_vars["algorithm"].set(f"编辑模式: {self.edit_mode}")

    def clear_temp(self):
        for a in self.temp_artists: a.remove()
        self.temp_artists=[]
        self.canvas.draw()
    ...
    def redraw(self):
        ...
        if self.selected_feature:
            ...
            if g['type']=='Point':
                ...
                self.ax.plot(np.array(g['coordinates'])[:,0],np.array(g['coordinates'])[:,1],'r',lw=3)
            elif g['type']=='Polygon':
    ...
    def add_field(self):
        ...
        if not name: return
        self.layers[self.selected_layer_idx] = add_property_field(self.layers[self.selected_layer_idx], name)
        self.refresh_prop()
    ...
    def del_field(self):
        ...
        self.layers[self.selected_layer_idx] = delete_property_field(self.layers[self.selected_layer_idx], field)
        self.refresh_prop()
    ...
    def batch_edit_prop(self):
        ...
        if not field or val is None: return
    ...
    def export_file(self):
        ...
        if not path: return
        layer = self.layers[self.selected_layer_idx] if self.selected_layer_idx!=None else self.layers[0]
        if fmt=="shp":
            save_shp(layer, path)
        else:
            save_dwg(layer, path)
    ...
    def on_mouse_down(self,e):
        ...
        x,y=e.xdata,e.ydata
        if e.button==3:
            self.drawing_points=[];self.clear_temp();return
        if self.edit_mode=="select": self._select(x,y)
        elif self.edit_mode=="move": self.move_start=(x,y)
        elif self.edit_mode=="draw_point": self._add_point(x,y)
        elif self.edit_mode in ["draw_line","draw_polygon"]:
            self.drawing_points.append((x,y));self._update_temp()

    def on_mouse_move(self,e):
        # 关键修复：添加严格的存在性检查
        if not (self.edit_mode=="move" 
                ...
                and 0 <= self.selected_feature_idx < len(self.layers[self.selected_layer_idx]['features'])):
            return
    ...
    def on_mouse_up(self,e):
        if self.edit_mode=="move": self.move_start=None

    def on_click(self,e):
        if e.dblclick and e.inaxes:
            if self.edit_mode=="draw_line" and len(self.drawing_points)>=2: self._finish_line()
            elif self.edit_mode=="draw_polygon" and len(self.drawing_points)>=3: self._finish_poly()

    def _select(self,x,y):
        self.selected_layer_idx,self.selected_feature_idx,self.selected_feature = select_feature(self.layers,x,y)
        self.redraw()
        if self.selected_feature:
            self.status_vars["features"].set(f"选中: 1个要素")
        else:
    ...
    def _update_temp(self):
        ...
        if len(self.drawing_points)>1:
            l,=self.ax.plot(np.array(self.drawing_points)[:,0],np.array(self.drawing_points)[:,1],'r-')
    ...
    def _finish_line(self):
        ...
        self.drawing_points=[];self.clear_temp();self.redraw()
        self.status_vars["features"].set(f"总要素: {sum(len(l['features']) for l in self.layers)}")
