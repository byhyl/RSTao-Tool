...
class PluginDialog(ctk.CTkToplevel):
    ...
    def _create_ui(self):
        ctk.CTkLabel(self, text="插件管理", font=("Microsoft YaHei UI", 18, "bold")
                    ).pack(pady=(16, 4))
        ctk.CTkLabel(self, text="已安装的扩展插件", font=FONT_SMALL,
                    text_color=THEME["text_secondary"]).pack(pady=(0, 10))

        self.plugin_list = ctk.CTkScrollableFrame(self, height=220, fg_color=THEME["card"],
                                                   corner_radius=8)
        ...
        ctk.CTkLabel(info_card, text="安装插件: 在 plugins/ 下创建子文件夹，放入 plugin.json + Python 模块即可",
                    font=FONT_SMALL, text_color=THEME["text_muted"]).pack(padx=14, pady=10)

        btn_row = ctk.CTkFrame(self, fg_color="transparent")
        btn_row.pack(pady=8)
        ctk.CTkButton(btn_row, text="刷新列表", command=self._refresh_list,
                     width=90, font=FONT_SMALL).pack(side="left", padx=4)
        ctk.CTkButton(btn_row, text="打开插件目录", command=self._open_dir,
                     width=110, font=FONT_SMALL,
    ...
    def _refresh_list(self):
        ...
        if not plugins:
            ctk.CTkLabel(self.plugin_list, text="暂无插件", font=FONT_NORMAL,
        ...
        for p in plugins:
            ...
            ctk.CTkLabel(row, text=f"v{p.version}", font=FONT_SMALL,
