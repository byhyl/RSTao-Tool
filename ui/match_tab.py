...
class MatchTab(ctk.CTkFrame):
    ...
    def build_control(self):
        ...
        self.template_listbox = ctk.CTkTextbox(self.template_card.content, height=4, font=FONT_SMALL)
        ...
        self.search_label = ctk.CTkLabel(
            self.search_card.content, text="未选择文件", wraplength=250, text_color=THEME["text_secondary"], font=FONT_SMALL
        )
        ...
        ctk.CTkLabel(self.param_card.content, text="匹配阈值", font=FONT_NORMAL).pack(anchor="w", pady=(5,2), padx=5)
        self.match_threshold = ctk.DoubleVar(value=DEFAULT_MATCH_THRESHOLD)
        threshold_scale = ctk.CTkSlider(
            self.param_card.content, from_=0.0, to=1.0, variable=self.match_threshold,
        ...
        ctk.CTkLabel(self.param_card.content, text="非极大值抑制阈值", font=FONT_NORMAL).pack(anchor="w", pady=(5,2), padx=5)
        self.nms_threshold = ctk.DoubleVar(value=DEFAULT_NMS_RADIUS/10)
        nms_scale = ctk.CTkSlider(
            self.param_card.content, from_=0.0, to=1.0, variable=self.nms_threshold,
        ...
        self.template_count_label = ctk.CTkLabel(self.result_card.content, text="模板数量：0", font=FONT_SMALL)
        self.template_count_label.pack(anchor="w", pady=2, padx=5)
        self.match_count_label = ctk.CTkLabel(self.result_card.content, text="匹配成功：0", font=FONT_SMALL)
    ...
    def run_single_matching(self):
        try:
            ...
            self.result_img = self.core.draw_single_match_result(self.search_img, res, t_name, t_color)
    ...
    def run_single_multi_matching(self):
        try:
            t_img, t_name, t_color = self.templates[0]
            res = self.core.single_multi_matching(t_img, self.search_img, self.match_threshold.get(), self.nms_threshold.get())
    ...
    def run_multi_target_matching(self):
        try:
            res = self.core.multi_target_matching(self.templates, self.search_img, self.match_threshold.get())
            ...
            self.match_count_label.configure(text=f"匹配成功：{success_count}/{len(self.templates)}")
